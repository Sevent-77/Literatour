...
<div>
    ...
    if(Livros::checaResenha($dados->id_livro)):
        ...
        foreach ($resenhas as $resenha): ?>
            <h3><?= $resenha->user_nome ?></h3>
            <h3><?= $resenha->resen_horario ?></h3>
            <p><?= $resenha->resen_texto ?></p>
            <h4>Respostas</h4>
            <form action="" method="post">
                <textarea name="" id="" cols="100" rows="2"></textarea>
            </form>
            ...
            if(Livros::checaResposta($resenha->id_resenha)):?>
                ...
                foreach ($respostas as $resposta): ?>
                    <h3><?= $resposta->user_nome ?></h3>
                    <h3><?= $resposta->resp_horario ?></h3>
                    <p><?= $resposta->resposta_text ?></p>
                <?php 
                endforeach;
                echo '</div>';
            endif;
